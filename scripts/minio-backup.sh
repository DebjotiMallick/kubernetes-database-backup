...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] ⚡ Initiating database backup process for Milvus in $BUCKET_NAME bucket..."

# Read YAML entries into an array
readarray -t db_array < <(yq -r ".${DBTYPE}[] | @json" /configs/databases.yaml)

# Milvus-specific backup logic
for db in "${db_array[@]}"; do
    ...
    CA_FILE=$(echo "$db" | jq -r '.tls? | .ca_file // ""')

    echo "===================================================================="
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] 📢 Preparing database backup process for Milvus in $NAMESPACE namespace on $HOSTNAME..."
...
log:
  ...
  console: false
  file:
    rootPath: "/backups/$DBTYPE/logs/milvus_backup.log"
http:
...
milvus:
  ...
  password: "Milvus"
  tlsMode: 1
  caCertPath: /backups/${DBTYPE}/ca.crt
  serverName: ${HOSTNAME}.${NAMESPACE}
  mtlsCertPath: ""
  mtlsKeyPath: ""
minio:
  storageType: "minio"
  address: ${HOSTNAME}-minio.${NAMESPACE}
  port: 9000 
  ...
  backupAddress: ${HOSTNAME}-minio.${NAMESPACE}
  backupPort: 9000 
...
EOF
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] 🛠️ Milvus config.yaml created successfully."
    ...
    kubectl exec -n ${NAMESPACE} ${POD_NAME} -- cat ${CERT_DIR}/${CA_FILE} > /backups/${DBTYPE}/ca.crt
    ...
    START_TIME=$(date +%s)
    milvus-backup create -n $FILENAME
    BACKUP_STATUS=$?
    END_TIME=$(date +%s)
    mc alias set minio http://${HOSTNAME}-minio.${NAMESPACE}:9000 minioadmin minioadmin > /dev/null 2>&1
    mc cp --recursive minio/milvus-bucket/backup/$FILENAME /backups/${DBTYPE}/  > /dev/null 2>&1
    tar -zcf /backups/${DBTYPE}/$FILENAME.gz /backups/${DBTYPE}/$FILENAME > /dev/null 2>&1
    mc rm --recursive --force minio/milvus-bucket/backup/$FILENAME > /dev/null 2>&1
    gunzip -t "/backups/${DBTYPE}/$FILENAME.gz"
    BACKUP_INTEGRITY=$?

    # Check backup status
    if [ $BACKUP_STATUS -eq 0 ] && [ $BACKUP_INTEGRITY -eq 0 ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ✅ Milvus backup completed successfully"
        ...
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ⚠️ Error: Milvus backup failed for $HOSTNAME in $NAMESPACE namespace with status $BACKUP_STATUS. Please check the logs for more information."
        ((FAILURE_COUNT++))
    fi
