...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] ⚡ Initiating database backup process for MariaDB in $BUCKET_NAME bucket..."

# Read YAML entries into an array
readarray -t db_array < <(yq -r ".${DBTYPE}[] | @json" /configs/databases.yaml)

# MariaDB-specific backup logic
for db in "${db_array[@]}"; do
    HOSTNAME=$(echo "$db" | jq -r '.hostname')
    NAMESPACE=$(echo "$db" | jq -r '.namespace')
    
    echo "===================================================================="
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] 📢 Preparing database backup process for MariaDB in $NAMESPACE namespace on $HOSTNAME..."
    ...
    export USER=root
    export PASSWORD=$(kubectl exec -n "$NAMESPACE" "$POD_NAME" -c $CONTAINER_NAME -- printenv MARIADB_ROOT_PASSWORD 2>/dev/null || echo "")
    if [[ -n "$USER" && -n "$PASSWORD" ]]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] 🔑 MariaDB credentials retrieved successfully"
    else
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ⚠️ Missing MariaDB credentials, skipping this instance..."
        ((FAILURE_COUNT++))
        continue 
    fi
    ...
    if [ $BACKUP_STATUS -eq 0 ] && [ $BACKUP_INTEGRITY -eq 0 ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ✅ MariaDB backup completed successfully"
        ...
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ⚠️ Error: MariaDB backup failed for $HOSTNAME in $NAMESPACE namespace with status $BACKUP_STATUS. Please check the logs for more information."
        ((FAILURE_COUNT++))
    fi
